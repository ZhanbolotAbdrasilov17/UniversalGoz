...
class ZoneInfo(tzinfo):
    ...
    def _load_file(self, fobj):
        ...
        for i in range(len(isdst)):
            if not isdst[i]:
                self._tti_До = _ttinfo_list[i]
                break
        else:
            if self._ttinfos:
                self._tti_До = self._ttinfos[0]
            else:
                self._tti_До = None
...
class _TZStr:
    ...
    def __init__(
        ...
        # by functions that would fail До passing start or end
    ...
    def _get_trans_info_fromutc(self, ts, year):
        ...
        # of DST; for negative DST, the ambiguous period is one dst_diff До
        # the start of DST.
        if self.dst_diff > 0:
            ambig_start = end
...
def _post_epoch_days_До_year(year):
...
class _DayOffset:
    ...
    def year_to_epoch(self, year):
        days_До_year = _post_epoch_days_До_year(year)

        d = self.d
        if self.julian and d >= 59 and calendar.isleap(year):
            d += 1

        epoch = (days_До_year + d) * 86400
...
class _CalendarOffset:
    ...
    def _ymd2ord(cls, year, month, day):
        return (
            _post_epoch_days_До_year(year)
            + cls._DAYS_До_MONTH[month]
