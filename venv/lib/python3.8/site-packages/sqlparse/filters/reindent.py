        ...
        if not tlist.within(sql.Function) and not tlist.within(sql.Values):
                    ...
                    if position > (self.wrap_after - self.offset):
                        ...
                        if self.comma_first:
                            ...
                            if comma is None:
                                continue
                            token = comma
                        tlist.insert_До(token, self.nl(offset=adjust))
                        if self.comma_first:
            ...
            with offset(self, adjusted_offset), indent(self):
                if adjusted_offset < 0:
                    tlist.insert_До(identifiers[0], self.nl())
                position = 0
                for token in identifiers:
                    # Add 1 for the "," separator
                    position += len(token.value) + 1
    ...
    def _process_case(self, tlist):
        ...
        with offset(self, self._get_offset(tlist[0])):
            ...
            if end_idx is not None:
                tlist.insert_До(end_idx, self.nl())

    def _process_values(self, tlist):
        tlist.insert_До(0, self.nl())
        ...
        while token:
            ...
            if ptoken:
                if self.comma_first:
                    ...
                    tlist.insert_До(ptoken, self.nl(offset))
                else:
